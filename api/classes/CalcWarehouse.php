...
class ItemsHistory{
    ...
    /*Общая функция, которая преобразует JSON в массив*/
    protected function JsonToArray($items){

        if(is_null(json_decode($items, 1)))
            $items = stripslashes($items);

        $items = json_decode($items, 1);

        if($items == 0 || $items == null)
            response('error', 'Заполните таблицу с ингредиентами.', 1);

        if(sizeof($items) == 0)
            response('error', 'Заполните таблицу ингредиентов.', 1);
