...
use Support\ItemsClass;
use Support\mDB;
use Support\OrderClass;
use Support\Pages;
use Support\Request;
...
function SelectionCriteria($filter){
    ...
    if($filter == null || sizeof($filter) == 0)
         Utils::response('error', array('msg' => 'Фильтр имеет неверный формат.'), '395');
...
function DifferentPrice($product, $technical_card, $user, $different_price, $global_price, $create = false){
    ...
    if(!$points = DB::escape($request['points'])){
        if($create){
            DB::delete(DB_PRODUCT_COMPOSITION, 'technical_card = '.$technical_card);
            DB::delete(DB_TECHNICAL_CARD, 'id = '.$technical_card);
        }
        Utils::response('error', array('msg' => 'На одной или более точках не указана цена.'), '365');
    }
    ...
    if($points == null || sizeof($points) == 0){
        ...
        Utils::response('error', array('msg' => 'На одной или более точках не указана цена.'), '366');
    }
...
//Методы для работы с тех. картами
class TechnicalCardsController
{


   public static function add(){

    $userToken = Request::authUser();
    $request = Request::$request;
    
        if(!$product = DB::escape($request['product_id'])) 
            Utils::response('error', array('msg' => 'Выберите продукт, для которого хотите создать тех. карту.'), '336');
        ...
        if(DB::getRecordCount($productData) == 0)
            Utils::response('error', array('msg' => 'Такого товара не существует.'), '337');

        $productData = DB::getRow($productData);

        $fields['product'] = $product;

        if(!$bulk_untils = DB::escape($request['bulk_untils']))
            Utils::response('error', array('msg' => 'Выберите единицы измерения.'), '340');


        if(!$bulk_value = DB::escape($request['bulk_value']))
            Utils::response('error', array('msg' => 'Укажите кол-во '.$bulk_untils), '339');

        if($subname = DB::escape($request['subname']))
            $fields['subname'] = $subname;

        if($name_price = DB::escape($request['name_price']))
            $fields['name_price'] = $name_price;

        $cashback_percent = DB::escape($request['cashback_percent']);
        if($cashback_percent < 0 || $cashback_percent > 100)
            Utils::response('error', 'Процент кэшбэка должен находиться в пределах от 0 до 100.', 422);
        ...
        if($minutes = DB::escape($request['minutes'])){
            if($minutes < 0)
                $minutes = 0;
        ...
        if($seconds = DB::escape($request['seconds'])){
        ...
        if($cooking_method = DB::escape($request['cooking_method']))
            $fields['cooking_method'] = $cooking_method;

        if($color = DB::escape($request['color'])){
            $colorData = DB::select('id', DB_COLORS, 'id = '.$color);

            if(DB::getRecordCount($colorData) == 0)
                Utils::response('error', array('msg' => 'Такого цвета не существует.'), '332');

            $fields['color'] = $color;

        }

        if($different_price = DB::escape($request['different_price']))
            $fields['different_price'] = $different_price; 

        if($not_promotion = DB::escape($request['not_promotion']))
            $fields['not_promotion'] = $not_promotion;

         if(isset($request['weighted'])){
             $fields['weighted'] = (int)$request['weighted'];
         }   

        $fields['price'] = (DB::escape($request['price'])) ? DB::escape($request['price']) : 0;
        ...
        if(DB::getRecordCount($technical_card_data) != 0) {
            ...
            if($arch == null)
                Utils::response('error', array('msg' => 'Такая тех. карта уже существует.'), '538');
            else
                Utils::response('error', array('msg' => 'Данная тех. карта находится в архиве, восстановите её.'), '538');
        }
        ...
        mDB::collection("technical_cards")->insertOne($fields);
        ...
        Utils::response('success', array('msg' => 'Техническая карта создана.'), '620');
    ...
    public static function validation(){
        ...
        Utils::response('success', $newItems, '7');
      ...
      public static function get(){
        ...
        $sorting = OrderClass::technical_cards(Pages::$field, Pages::$order);

        //Критерии выборки
        if($filter = DB::escape($request['filter']))
            $filter = SelectionCriteria($filter);

        //Поисковый запрос
        if($search = DB::escape($request['search']))
        ...
        if($categories = DB::escape($request['categories']))
            $categories = CategoriesFilter($categories);

        if(!$point = DB::escape($request['point']))
            Utils::response('error', array('msg' => 'Выберите заведение.'), '329');

        $archive = '
            AND tc.id '.(DB::escape($request['archive']) ? '' : 'NOT').' IN (
                SELECT product_id
                FROM '.DB_ARCHIVE.'
        ...
        Utils::response('success', $result, '7', $pageData);
    }
    ...
    public static function composition(){
        ...
        if((int)Pages::$page == 1){
            ...
            $begin = $element_count*(int)Pages::$page - $element_count;
            $limit = $begin.','.$element_count; 
        }

        if(!$technical_card = DB::escape($request['technical_card']))
            Utils::response('error', array('msg' => 'Выберите тех. карту.'), '371');
    ...
    public static function  info(){
        ...
        if(!$technical_card = DB::escape($request['id']))
            Utils::response('error', array('msg' => 'Выберите тех. карту.'), '371');

        $technical_card_data = DB::query('SELECT tc.id, tc.code, tc.product, tc.weighted, pr.name, tc.subname, tc.name_price, tc.bulk_value, tc.bulk_untils, tc.price, tc.preparing_minutes, tc.preparing_seconds, tc.color, c.code AS color_code, tc.different_price, tc.not_promotion, tc.cooking_method, tc.partner, tc.cashback_percent
                                            FROM '.DB_TECHNICAL_CARD.' tc
        ...
        if(DB::getRecordCount($technical_card_data) == 0)
            Utils::response('error', array('msg' => 'Тех. карта не найдена.'), '389');
        ...
        $result = array('id' => $row['id'],
                        ...
                        'color' => array('id' => $row['color'],
                                        'code' => $row['color_code']),
                        'different_price' => $editing_allowed ? (bool)$row['different_price'] : true,
        ...
        mDB::collection("technical_cards")->insertOne($result);
     ...
     public static function delete(){
        ...
        if(DB::delete(DB_TECHNICAL_CARD, 'id = '.$technical_card))
            Utils::response('success', array('msg' => 'Тех. карта удалена.'), '627');
        else
            Utils::response('error', '', '503');
      ...
      public static function edit(){
        ...
        if(!$product = DB::escape($request['product_id'])) 
            Utils::response('error', array('msg' => 'Выберите Товар.'), '336');

        $productData = DB::select('id, partner, image', DB_PRODUCTS, 'id = '.$product.' AND (partner = '.$userToken['id'].' OR partner IS NULL)');

        if(DB::getRecordCount($productData) == 0)
            Utils::response('error', array('msg' => 'Товар не найден.'), '337');

        if($code = DB::escape($request['code'])){
            if(strlen($code) > 32)
                Utils::response('error', array('msg' => 'Штрих-код не может превышать больше 32-х символов.'), '588');
            $fields['code'] = $code;
        }
        ...
        if(!$bulk_value = DB::escape($request['bulk_value']))
            Utils::response('error', array('msg' => 'Укажите '.$untils), '339');
        ...
        if($editing_allowed){
            ...
            $fields['product'] = $product;
            $fields['subname'] = DB::escape($request['subname']);
            $fields['name_price'] = DB::escape($request['name_price']);
            $fields['cashback_percent'] = $cashback_percent;
            $fields['bulk_value'] = $bulk_value;
            $fields['bulk_untils'] = $bulk_untils;
            $fields['preparing_seconds'] = 0;
            ...
            //$fields['different_price'] = (DB::escape($request['different_price'])) ? 1 : 0;
            $different_price = DB::escape($request['different_price']);
            $fields['different_price'] = $different_price ? 1 : 0;

            if($not_promotion = DB::escape($request['not_promotion']))
                $fields['not_promotion'] = $not_promotion ? 1 : 0;
            ...
            //$fields['not_promotion'] = (DB::escape($request['not_promotion'])) ? 1 : 0;
            $fields['price'] = (DB::escape($request['price'])) ? DB::escape($request['price']) : 0;
        ...
        Utils::response('success', array('msg' => 'Техническая карта изменена.'), '628');

    }


    public static function composition_description(){
        $userToken = Request::authUser();
        
        $request = Request::$request;

        if(!$id = DB::escape($request['id']))
            Utils::response('error', 'Не передан ID тех. карты.', 422);

        if(!$description = DB::escape($request['description']))
            $description = '';
        ...
        Utils::response('success', 'Изменения сохранены.', 201);
