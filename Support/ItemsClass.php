...
class ItemsClass
{
    ...
    public function validate()
    {
        ...
        if (!$items = DB::escape($request['items']))
            Utils::response('error', 'Укажите состав продукции.', 1);



        if (sizeof($items) == 0) // Проверяем на пустоту
            Utils::response('error', array('msg' => 'Заполните таблицу с ингредиентами.'), '354');
        ...
        for ($i = 0; $i < sizeof($items); $i++) {
            ...
            if (!$items[$i]['id'])
                Utils::response('error', array('msg' => 'Таблица с ингредиентами заполнена неверно. Строка ' . $row_num . '.'), '355'); // В массиве у каждого объекта должен быть id

            if ($items[$i]['untils'] == 'шт' && !$items[$i]['count'] && $this->create)
                Utils::response('error', array('msg' => 'Укажите количество. Строка ' . $row_num . '.'), 422);

            /*             if($items[$i]['untils'] != 'шт' && !$items[$i]['net_mass'] && $this->create)
                Utils::response('error', array('msg' => 'Укажите массу нетто. Строка '.$row_num.'.'), 422); */
        ...
        $QueryItems = DB::query('SELECT i.*, AVG(pi.price) AS cost_price
                                    ...
                                    WHERE ' . implode(' OR ', $itemsWhere) . '
                                    GROUP BY i.id');

        if (DB::getRecordCount($QueryItems) != sizeof($items))
            Utils::response('error', array('msg' => 'Один или более ингредиентов не существуют.'), '355'); // Если запрашиваемое и полученное количество ингредиентов не сходится, то ошибка
        ...
        while ($row = DB::getRow($QueryItems)) {
            ...
            for ($i = 0; $i < sizeof($items); $i++) {
                ...
                if ($row['id'] == $items[$i]['id']) {
                    ...
                    $items[$i]['cleaning'] = ($items[$i]['cleaning_checked'] == 1 && $row['untils'] != 'шт') ? (float)$row['cleaning'] : 0;
                    $items[$i]['cooking'] = ($items[$i]['cooking_checked'] == 1 && $row['untils'] != 'шт') ? (float)$row['cooking'] : 0;
                    $items[$i]['frying'] = ($items[$i]['frying_checked'] == 1 && $row['untils'] != 'шт') ? (float)$row['frying'] : 0;
                    $items[$i]['stew'] = ($items[$i]['stew_checked'] == 1 && $row['untils'] != 'шт') ? (float)$row['stew'] : 0;
                    $items[$i]['bake'] = ($items[$i]['bake_checked'] == 1 && $row['untils'] != 'шт') ? (float)$row['bake'] : 0;
